...
2. 등록됐지만 URL 불일치 → 사용자에게 alert + exit 10
3. 등록됐고 URL 일치 → no-op
...
| `/sds-workflow:autopilot PROJ-XXXX [PROJ-YYYY ...]` | 자율 운행 — pick → 구현 → ship 원샷. 다중 이슈 시 worktree 격리 + subagent 병렬 |
...
| `Bash(bash:*)` 또는 `Bash(${CLAUDE_PLUGIN_ROOT}/scripts/*.sh:*)` | ship |
